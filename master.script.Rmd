...
```{r subset.normbrca.dataset, eval = FALSE}
...
```
```{r sctransform.visualize.clusters}
plot1<-DimPlot(norm.brca.pdx.integrated, group.by = "integrated_snn_res.0.1",label = T)
plot2<-DimPlot(norm.brca.pdx.integrated, group.by = "integrated_snn_res.0.5",label = T)
plot3<-DimPlot(norm.brca.pdx.integrated, group.by = "integrated_snn_res.1",label = T)
...
```
```{r sctransform.visualize.markers}
top10 <- norm.brca.pdx.integrated.res0.5.RNA.markers %>% group_by(cluster) %>% top_n(n = 10, wt = avg_logFC)
DoHeatmap(norm.brca.pdx.integrated, features = top10$gene, assay = "RNA") + NoLegend()
norm.brca.pdx.integrated.res0.5.RNA.markers %>% group_by(cluster) %>% top_n(n = 2, wt = avg_logFC)
```