...
ggplot(metabolism.interest.box.plot.matrix.melted,
                                    aes(x=cell.type,
                                        y=score,
                                        fill=status,
                                        linetype=individual))+
...
##Add 10X Chemistry Meta Data

```{r add.meta.data}
chemistry.meta.data<-list()
chemistry.meta.data[1:nrow(Norm.BRCA@meta.data)]<-"10X V2"
chemistry.meta.data[grep("ind1",Norm.BRCA@meta.data$individual)]<-"10X V1"
chemistry.meta.data[grep("ind2",Norm.BRCA@meta.data$individual)]<-"10X V1"
chemistry.meta.data<-as.character(chemistry.meta.data)

Norm.BRCA[["chemistry"]]<-chemistry.meta.data

metabolism.interest.box.plot.matrix<- cbind(Norm.BRCA@meta.data[,13], 
                                            Norm.BRCA@meta.data[,8], 
                                            Norm.BRCA@meta.data[,4],
                                            Norm.BRCA@meta.data$chemistry,
                                            Norm.BRCA@meta.data[,c(33,34,36,37,40,41,44,47,58,60,61,62)])
metabolism.interest.box.plot.matrix.melted<-melt(metabolism.interest.box.plot.matrix)
...
##Calculate % Genes Expressed by Individual and by Chemistry

```{r calculate.percent}

overall.metabolism.percent<-list()
for (i in 1:length(metabolism.pathways)){
              
    overall.metabolism.percent[i]<-(length(which(rownames(Norm.BRCA)%in%metabolism.pathways[[i]][[1]]))/length(metabolism.pathways[[i]][[1]]))
    
  } 
names(overall.metabolism.percent)<-names(metabolism.pathways)

v1.chemistry.cols<-colnames(Norm.BRCA)[grep("10X V1", Norm.BRCA@meta.data$chemistry)]
v2.chemistry.cols<-colnames(Norm.BRCA)[grep("10X V2", Norm.BRCA@meta.data$chemistry)]

counts.matrix<-GetAssayData(Norm.BRCA,assay = "RNA")
counts.matrix<-as.matrix(counts.matrix)

v1.matrix<-counts.matrix[,v1.chemistry.cols]
v2.matrix<-counts.matrix[,v2.chemistry.cols]
