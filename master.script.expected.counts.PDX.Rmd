...
```{r load.objects, echo=FALSE,eval=FALSE}
#HCI001
...
## Subset Norm/BRCA Dataset to Epithelial Cells Only
...
```
```{r check columns and the epithelial subset, eval=FALSE}
...
```
```{r check columns and the epithelial subset,eval=FALSE,echo=FALSE}
colnames(norm.brca.epithelial.individual.1.objects@meta.data)
head(norm.brca.epithelial.individual.1.objects@meta.data)
```

##Generate a list of Seurat objects (PDX models and Norm/BRCA Individuals to perform the standard workflow on
...
```{r standard.workflow.normalize.objects, eval=FALSE, echo=FALSE}
...
```{r standard.workflow.integrate.data, echo = FALSE, eval = FALSE}
#After 
...
```{r visualize.integration.matrix, echo = FALSE, eval = FALSE}
...
```{r standard.workflow.integration.visualization.clustering, echo = FALSE, eval = FALSE}
...
UMAP.integrated.data <- load("/Users/paigehalas/Desktop/UMAP.integrated.data.rds")
